#include "displayapp/screens/LivingRoomControl.h"
#include <cstdint>
...
static void event_handler(lv_obj_t* obj, lv_event_t event) {
  LivingRoomControl* screen = static_cast<LivingRoomControl*>(obj->user_data);
  screen->OnObjectEvent(obj, event);
}

LivingRoomControl::LivingRoomControl(Pinetime::Controllers::RemoteControlService& remoteControl) 
...
LivingRoomControl::~LivingRoomControl() {
  lv_obj_clean(lv_scr_act());
}

void LivingRoomControl::OnObjectEvent(lv_obj_t* obj, lv_event_t event) {
  if (event == LV_EVENT_VALUE_CHANGED) {
    if (obj == slider1) {
      uint8_t val = lv_slider_get_value(slider1);
      lv_label_set_text_fmt(label1, "Lamp 1: %d", val);
      uint8_t data[2] = {0x02, val};
